        ...
        <div className='w-[95%] border-b-2  border-gray-200 pb-2'>
            {
               primaryList.map((item,pos)=>{
                return (
                  <div onClick={()=>{router.replace(`/home/user?tab=${item.tag.toLowerCase()}`)}} className={`font-semibold mt-[1px] mb-[1px] p-3 w-[100%] rounded-lg transition-all duration-100 cursor-pointer flex items-center gap-2 ${item.tag.toLowerCase()==tabParams?.tab?"text-[white]  bg-[#9a2143] ":" hover:bg-[#f7d5e0] hover:text-[black]"}`} key={pos}>
